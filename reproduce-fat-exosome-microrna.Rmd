...
wtvsko.lst <- analyze_thomou_qPCR(wtvsko.ct)
wtvsko.pheno <- wtvsko.lst$pheno; wtvsko <- wtvsko.lst$toptab
wtvsko.ndct <- wtvsko.lst$ndct; wtvsko.ndct.filt <- wtvsko.lst$ndct.filt
```

Compared to control, ADicerKO mice exhibited significant alterations in 422 circulating miRNAs. Of these, only 3 miRNAs had a significant increase, while 419 had significant decreases (Figures 1c-d, S1d and Supplemental Table 1) with 88% reduced by >4-fold and 13% reduced by >32-fold
```{r}
x <- sum(wtvsko$WTvsKO.p<0.05)
if (x==422) cat(":-)") else cat(x)
x <- sum(wtvsko$WTvsKO.p<0.05 & wtvsko$WTvsKO.logFC<0)
if (x==3) cat(":-)") else cat(x)
x <- sum(wtvsko$WTvsKO.p<0.05 & wtvsko$WTvsKO.logFC>0)
if (x==419) cat(":-)") else cat(x)
x <- round(sum(wtvsko$WTvsKO.p<0.05 & wtvsko$WTvsKO.FC>4)/419, 2)
if (x==0.88) cat(":-)") else cat(x)
x <- round(sum(wtvsko$WTvsKO.p<0.05 & wtvsko$WTvsKO.FC>32)/419, 2)
if (x==0.13) cat(":-)") else cat(x)
```

write supp table 1  
```{r st1}
st.cols <- c("p", "FDR", "logFC", "FC")
write.csv(wtvsko[,paste0("WTvsKO.", st.cols)], "Supplemental_tables/ST1_ADicerKO-6mo-stats.csv")
```

Heatmap Fig 1c  
```{r fig1c}
heatmap(t(wtvsko.ndct.filt), col=greenred(50), scale="column", Rowv=NA, Colv=NA)
...
Indeed, in sample of 80 miRNAs of which 79 were detected there was a 50-75% reduction in total serum miRNAs in ADicerKO serum when compared to total serum of WT mice (Extended Data Figure 2a)  
```{r s1e, echo=FALSE}
#######################################################################################################################
##serum
#######################################################################################################################
...
x <- nrow(serum.ndct)
if (x==80) cat(":-)") else cat(x)
x <- nrow(serum.ndct.filt)
if (x==79) cat(":-)") else cat(x)
all(serum.toptab$KOvsWTinSer.logFC>0)

heatmap(t(serum.ndct.filt), Rowv=NA, Colv=NA, col=greenred(50))
serum.ndct.filt <- scale(x=t(serum.ndct.filt))
write.csv(t(serum.ndct.filt), "heatmap_z-scores/serum_ndct_z.csv")
```

Thus, brown preadipocytes isolated from Dicer-floxed animals and recombined by transduction with Ad-Cre exhibited marked reductions of at least 2 fold difference in 380 miRNAs profiled of which 359 were detectable in 91% of the exosomal miRNAs released in culture supernatants when compared to control Ad-GFP transduced cells (Extended Data Figure 2b).
```{r s2b}
#######################################################################################################################
##BAT preads
#######################################################################################################################
...
heatmap(t(bat.pread.ndct.filt), Rowv=NA, Colv=NA, col=greenred(50))
...
Genome-wide qPCR profiling of 572 miRNAs in human sera revealed 119 significantly different between control and HIV lipodystrophy subjects and 213 significantly different between control and CGL subjects (Figures 1e-f, Extended Data Figure 3b, Supplemental Tables 2 and 3).  
```{r}
...
heatmap 1e  
```{r 1e, echo=FALSE}
heatmap(t(human.ndct), Rowv=NA, Colv=NA, scale="column", col=greenred(50))
...
PCA S3b  
```{r s3b}
s1i <- multi.pca(human.ndct, human.pheno, color='grp', shape='grp', name=NA, all.size=3, rm.leg.title=TRUE)
```
...
Of these, only 5% (29 miRNAs) were upregulated in either HIV or CGL patients, while 217 (38%) were robustly decreased in either CGL or HIV lipodystrophy, and 75 were decreased in both lipodystrophy groups (Figure 1g, Supplemental Table 4)   
```{r st4}
...
mmu.nms <- rownames(wtvsko)[wtvsko$WTvsKO.p<0.05 & wtvsko$WTvsKO.logFC>0]
...
miRNA profiling of circulating exosomes from 4 week-old ADicerKO mice (Extended Data Figure 3c) demonstrated that of the 380 profiled miRNAs, 373 miRNAs were detectable, of which 43% (or 202 miRNAs) were down-regulated in ADicerKO mice and only 4% (23 miRNAs) were up-regulated when compared to Lox littermate controls.   
```{r}
...
heatmap(t(depot.heat.ndct.filt), col=greenred(50), scale="column", Rowv=NA, Colv=NA)
...
Considering only the miRNAs that were expressed in at least one fat depot, 126 were highly expressed in BAT, 106 in Ing-WAT, and 160 in Epi-WAT, with 82 of these miRNAs expressed in all three depots (Figures 2b, Extended Data 4a; Supplemental Table 6).
```{r}
...
#answer TT's question, and subtract one mir for u6
cat("TT question: we keep", sum(wt.expr)-1, "mirs that have enough WT expression.\n")
...
#TT used linear scale, but I think that only worked b/c most mirs in depot were *>* WT
...
As in the first cohort, in the sham operated ADicerKO mice circulating exosomal miRNAs were markedly reduced compared to controls (Figure 2c). By comparison, ADicerKO mice that received BAT and Ing-WAT transplants showed remarkable restoration of circulating exosomal miRNAs, with lower restoration in mice receiving Epi-WAT (Figures 2c and Extended Data Figure 5a; Supplemental Tables 7 and 8).  
```{r 2c}
##trans.ndct has all mirs where mean(WT)<34
heatmap(t(trans.ndct), col=greenred(50), scale="column", Rowv=NA, Colv=NA)
# my.heat(trans.ndct, name=NA, Colv=FALSE, labRow=NA, clip=10, margins=c(5,5), dendro="none")
...
if (all(colSums(rescue.mat)==c(150, 110, 116) & colnames(rescue.mat)==c("BAT", "EPI", "ING"))){
  cat(":-)")
} else { colSums(rescue.mat) }
```

Four miRNA candidates were identified (miR-99a, miR-99b, miR-100, and miR-466i), and three of these (miR-99a, -99b, and -100) were decreased by 75-80% in the serum of ADicerKO mice compared to controls. 
```{r fgf21}
##fgf21
mirs.tmp <- paste0("mmu-", c("miR-99a", "miR-99b", "miR-100", "miR-466i"))
trans.toptab[mirs.tmp,]
rescue.mat[intersect(mirs.tmp, rownames(rescue.mat)),]
```
