...
waterfall plots 1f  
```{r 1f}
barplot(sort(human.stats$HIVvsCon.logFC[human.stats$HIVvsCon.p < 0.05], decreasing=TRUE), main="HIV vs Control log2FC")
barplot(sort(human.stats$CGLvsCon.logFC[human.stats$CGLvsCon.p < 0.05], decreasing=TRUE), main="CGL vs Control log2FC")
```
...
rescue.depot.totals <- apply(vc[,1:3], MARGIN=2, FUN=function(v){ sum(vc[v==1, "Counts"]) })
